...
import com.nexse.serial.server.bean.SportwettenBettedEvent;
import com.nexse.serial.server.bean.SportwettenMarkSenseCard;
...
public class SportWetteMarkSenseCardsPrinter {
    private static final int LINE_FEED = 10;
       ...
       public static final String EVENT_X = "0";
    ...
    private static EventIntArraylistExchange printerExchange;
    private static SportWetteMarkSenseCardsPrinter instance;

    public SportWetteMarkSenseCardsPrinter() {

    }

    public static SportWetteMarkSenseCardsPrinter getInstance() {
        return instance;
    }

    public static void initialize(EventIntArraylistExchange exchange) {
        instance = new SportWetteMarkSenseCardsPrinter();
        betslipCounter = 1;
        printerExchange = exchange;
    }

    public  String printMarkSenseCard() throws Exception {
           MarkSenseCard mscToPrint = LastMarkSenseRead.getInstance().getCard();
           if (mscToPrint == null) {
               return "Print error no Mark Sense Card in memory";
           }

        SportwettenMarkSenseCard sportMsc = null;
        if (mscToPrint instanceof SportwettenMarkSenseCard) {
            sportMsc = (SportwettenMarkSenseCard) mscToPrint;
        } else {
            return "unknown mark sense card type";
        }

        System.out.println(sportMsc);
        ...
        for (SportwettenBettedEvent sportwettenBettedEvent : sportMsc.getBettedEvents()) {

            if (sportwettenBettedEvent.isTicked()) {
                addMarketToBetslip(sportwettenBettedEvent.getMarketCode(), sportwettenBettedEvent.getEventDate(), sportMsc.getStake(), sportwettenBettedEvent.getTeam1(), sportwettenBettedEvent.getTeam2(), sportwettenBettedEvent.getOdd(), sportwettenBettedEvent.getKurs().length != 0 ? sportwettenBettedEvent.getKurs()[0].toString() : "1");
            }
        }
           ...
           lineTotal.append("Total").append(padLeft(df.format(sportMsc.getTotalCost()),10,"*")).append(" ");
           ...
           linePayout.append("max. payout    :").append(padLeft(df.format(sportMsc.getTotalwinning()),12)).append(" ");
          ...
          printerExchange.put(dataToPrint);


           return betslipCompleteCode.toString();
       }

       private  void addMarketToBetslip(String code,Date eventDate,double stake,String team1,String team2,double bettedOdds,String bettedEvent) {
           addToDataToPrint(SET_RELATIVE_PRINTER_POS);
       ...
       private  void addToDataToPrint(Integer[] array) {
           dataToPrint.addAll(Arrays.asList(array));

       }

       private  void addToDataToPrint(ArrayList<Integer> array) {
              dataToPrint.addAll(array);
          }

       private  void addToDataToPrint(int value) {
       ...
       private  ArrayList<Integer> getPrintableByString(String toPrint) {
       ...
       private  String generateProgressiveMArketsId() {
     ...
     public  String padLeft(String s, int n) {
       ...
       public  String padLeft(String s, int n,String c) {
